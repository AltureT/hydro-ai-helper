...
3. **先保存配置**，确认“开始前检查”中的 AI 模型和运行环境均为正常；
4. 首次先运行“快速检查”，通过后再运行“全面检查”。

页面默认使用通俗的能力描述，不展示具体算法、沙箱或隐藏探针等实现细节；高级自定义和详细运行记录按需展开。开始前检查只读取已保存配置并探测运行环境，不会发起付费模型请求。若“测试数据生成”没有专用模型设置，则沿用通用 AI 设置。只有系统管理员可以调用，每小时最多运行 2 次，且每次都需要明确确认费用。结果不写入数据库，也不会自动上传。
