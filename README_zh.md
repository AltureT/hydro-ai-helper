...
收集**匿名统计数据**（安装数、活跃用户 7/30/90 天窗口、对话数、按日功能用量计数、版本），用于 GitHub 徽章和开发参考。

- 完全匿名（随机 UUID，无个人信息），域 ID 经 SHA-256 哈希
- 记录上报请求来源的**国家与省份**（由 Cloudflare 从服务器出口 IP 粗粒度推断，**不存储 IP 本身**，实例级粒度，用于地区分布统计）
- 不收集代码、对话内容或个人数据；实例记录 270 天未上报自动清理（覆盖寒暑假停机），按日功能用量保留 400 天
