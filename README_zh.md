...
- **批量 AI 学习总结** — 在作业/比赛成绩表页面一键为每位学生生成个性化学习总结
  - SSE 实时进度展示，支持停止、继续生成、重试失败学生
  - 智能提交采样：自动筛选关键里程碑（首次提交、首次 AC、分数提升等），保留学生调试思路和成长轨迹
  - 草稿/发布机制：教师审阅编辑后再发布给学生
  - 学生在成绩表页面自动查看已发布的学习总结，支持自动刷新
