...
export function normalizeReportMarkdown(content: string): string {
  ...
  while (thinkingStart.test(result)) {
    ...
    if (parser.parse(result, {})[0]?.type === 'code_block') break;
    const end = thinkingEnd.exec(result);
    if (!end) return ''; // An unfinished provider wrapper has no visible report.
  ...
  return result.split('\n').map((line, index) => protectedLines.has(index) ? line : line.replace(
    /^( {0,3}#{1,6})(?:(?:&#(?:0*32|0*160|x0*20|x0*a0);|&nbsp;))+/i,
    '$1 ',
  )).join('\n');
}
  ...
  while (index < code.length) {
    ...
    if (formatted && depth > 0) {
      ...
      if (formatted && code[index] === '{') {
        if (code[index + 1] === '{') { index += 2; continue; }
        depth++;
      }
    }
    index++;
 ...
 */
function normalizePythonBlanks(code: string, fenceIndent: string): string {
  const hints: string[] = [];
...
export function normalizeHomeworkMarkdown(content: string, fallbackLanguage = ''): string {
  ...
  for (const fence of fences.reverse()) {
    ...
    if (!isPythonLanguage(language)) continue;
    ...
    if (!close || !new RegExp(`^${fence.markup[0]}{${fence.markup.length},}$`).test(close)) continue;
    const original = lines.slice(start + 1, end - 1).join('\n');
    const migrated = normalizePythonBlanks(original, indent);
    if (migrated !== original) lines.splice(start + 1, end - start - 2, ...migrated.split('\n'));
  }
  return lines.join('\n');
}
