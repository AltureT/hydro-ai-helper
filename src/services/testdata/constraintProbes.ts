...
function structuralPredicate(
  expression: string,
  fieldId: string,
  vertexFieldId: string,
): { predicate: 'simpleGraph' | 'connected' | 'tree' | 'dag'; domainMin: 0 | 1 }
  | undefined {
  const escapedField = fieldId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const escapedVertexField = vertexFieldId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const match = new RegExp(
    `^(simpleGraph|connected|tree|dag)\\(${escapedField}, vertices=(0\\.\\.${escapedVertexField}-1|1\\.\\.${escapedVertexField})\\)$`,
  ).exec(expression);
  if (!match) return undefined;
  return {
    predicate: match[1] as 'simpleGraph' | 'connected' | 'tree' | 'dag',
...
function parseEdgeList(
  ...
  let edgeCountLocation: { line: number; token: number } | undefined;
  if (edgeCountFieldId) {
  ...
  if (vertexCount === undefined || vertexCount === 0
    || (edgeCountFieldId && edgeCount === undefined)) return 'MUTATION_NOT_ISOLATED';
...
function undirectedEdgeKey(edge: [number, number]): string {
  return edge[0] < edge[1] ? `${edge[0]}:${edge[1]}` : `${edge[1]}:${edge[0]}`;
}

function hasSelfLoop(edges: readonly [number, number][]): boolean {
  return edges.some(([left, right]) => left === right);
}

function hasDuplicateEdge(edges: readonly [number, number][], directed: boolean): boolean {
...
function hasDirectedCycle(layout: EdgeListLayout): boolean {
  const adjacency = new Map<number, number[]>();
  for (const [left, right] of layout.edges) {
...
function isSimpleUndirected(layout: EdgeListLayout): boolean {
  return !hasSelfLoop(layout.edges) && !hasDuplicateEdge(layout.edges, false);
}

function isTree(layout: EdgeListLayout): boolean {
...
function removeLine(input: string, line: number): Mutation | undefined {
  const lines = input.split('\n');
...
function appendEdgeLine(input: string, edge: [number, number], line: number): Mutation {
  const lines = input.split('\n');
  lines.splice(lines.length - 1, 0, `${edge[0]} ${edge[1]}`);
  return { input: lines.join('\n'), position: { line, token: 1 } };
}

function constructStructuralMutation(
  input: string,
  spec: ProblemSpecV1,
  target: Target,
  fieldId: string,
  ...
  const field = spec.inputFields.find(item => item.id === fieldId) as ProblemSpecV1['inputFields'][number];
...
interface OperationArgumentBounds {
  all: NumericBounds;
  nonTarget: NumericBounds;
  target?: NumericBounds;
}

function expressionReferencesField(expression: string, fieldId: string): boolean {
  const escaped = fieldId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`(^|[^A-Za-z0-9_.:-])${escaped}($|[^A-Za-z0-9_.:-])`)
    .test(expression);
}

function intersectBounds(current: NumericBounds, next: NumericBounds): NumericBounds | undefined {
  ...
  if ((min !== undefined && !Number.isSafeInteger(min))
    ...
    || (min !== undefined && max !== undefined && min > max)) return undefined;
  return {
    ...(min === undefined ? {} : { min }),
    ...(max === undefined ? {} : { max }),
  };
...
function resolveArgumentBounds(
  spec: ProblemSpecV1,
  target: Target,
  fieldId: string,
  source: InternalProbeRequest['source'],
  ...
  let targetBounds: NumericBounds | undefined;
  ...
  for (const constraint of spec.constraints) {
    ...
    if (!nextAll) return 'MUTATION_NOT_ISOLATED';
    all = nextAll;
    if (target.kind === 'constraint' && constraint.id === target.id) {
      if (targetBounds) return 'MUTATION_NOT_ISOLATED';
      targetBounds = bounds;
    } else {
      const nextNonTarget = intersectBounds(nonTarget, bounds);
      if (!nextNonTarget) return 'MUTATION_NOT_ISOLATED';
...
function valueSatisfiesBounds(value: number, bounds: NumericBounds): boolean {
...
function findMissingValue(
  ...
  if (bounds.min !== undefined) {
    ...
    if (valueSatisfiesBounds(candidate, bounds) && !present.has(candidate)) return candidate;
  }
...
function findTargetViolation(
  ...
  if (target.max !== undefined && target.max < Number.MAX_SAFE_INTEGER) {
    ...
    if (valueSatisfiesBounds(candidate, nonTarget)
      && !valueSatisfiesBounds(candidate, target)) return candidate;
  }
...
function parseOperations(
  input: string,
  spec: ProblemSpecV1,
): ParsedOperation[] | ConstraintProbeGap['reasonCode'] {
  const operationFields = spec.inputFields.filter(field => field.type === 'operations');
  if (operationFields.length !== 1) return 'UNPARSEABLE_ENCODING';
  ...
  if (!encoding) return 'UNPARSEABLE_ENCODING';
  const countFieldId = encoding[1];
  if (!operationField.dependsOn?.includes(countFieldId)) return 'DEPENDENCY_NOT_RESOLVED';
  ...
  if (count === undefined || lines.length !== count + 1) return 'MUTATION_NOT_ISOLATED';
  ...
  for (let index = 1; index < lines.length; index++) {
    ...
    if (!definition || (definition.name !== 'ADD' && definition.name !== 'DEL')
      || tokens.length !== definition.arguments.length + 1) return 'MUTATION_NOT_ISOLATED';
...
function operationArgumentIndex(
  spec: ProblemSpecV1,
...
function operationSupportsSetPresence(
  spec: ProblemSpecV1,
  operationName: 'ADD' | 'DEL',
  fieldId: string,
): boolean {
  const operation = (spec.operations || []).find(item => item.name === operationName);
  if (!operation || !operation.arguments.includes(fieldId)) return false;
...
function statefulViolations(
  ...
  const violations: Array<{ line: number; name: 'ADD' | 'DEL' }> = [];
  for (const operation of operations) {
    ...
    if (operation.name === 'ADD') {
      if (present.has(value)) violations.push({ line: operation.line, name: operation.name });
...
function presentBefore(
  ...
  for (let index = 0; index < beforeIndex; index++) {
    ...
    const value = operation.arguments[argumentIndex];
    if (operation.name === 'ADD') {
      if (present.has(value)) return undefined;
      present.add(value);
    } else {
      if (!present.has(value)) return undefined;
      present.delete(value);
...
function constructOperationMutation(
  input: string,
  spec: ProblemSpecV1,
  target: Target,
  fieldId: string,
  ...
  if (kind === 'add-existing-object') {
    ...
    if (!sourceViolations || sourceViolations.length !== 0) return 'MUTATION_NOT_ISOLATED';
    for (let index = 0; index < operations.length; index++) {
      if (operations[index].name !== selectedName) continue;
  ...
  if (targetIndex < 0 || replacement === undefined) return 'MUTATION_NOT_ISOLATED';
  const selected = operations[targetIndex];
  const mutation = replaceToken(
    input,
    { line: selected.line, token: argumentIndex + 2 },
    String(replacement),
  );
  if (!mutation) return 'MUTATION_NOT_ISOLATED';
...
function constructSubtaskUpperBoundMutation(
  input: string,
  spec: ProblemSpecV1,
  target: Target,
  fieldId: string,
...
function deduplicateRequests(requests: ValidatorProbeRecipe[]): ValidatorProbeRecipe[] {
...
function deriveConstructionRequests(
  spec: ProblemSpecV1,
  target: Target,
): ValidatorProbeRecipe[] {
  ...
  for (const field of spec.inputFields) {
    ...
    if (field.type === 'graph' || field.type === 'tree') {
      ...
      const predicate = vertexFieldId
        ...
        : undefined;
      const kinds: ValidatorProbeConstructionKind[] = predicate?.predicate === 'simpleGraph'
  ...
  if (stateful
    ...
    && operationSupportsSetPresence(spec, stateful[1] as 'ADD' | 'DEL', stateful[3])) {
    requests.push({
      targetId: target.id,
...
function resolveRecipeField(
  spec: ProblemSpecV1,
  target: Target,
  recipe: ValidatorProbeRecipe,
): ValidatorProbeRecipe | undefined {
  if (recipe.fieldId) return { ...recipe };
  const compatible = compatibleFieldsForConstruction(spec, recipe.constructionKind);
  const referenced = compatible.filter(field => expressionReferencesField(
    target.expression,
    field.id,
  ));
...
function sequenceSnapshot(
  ...
  spec: ProblemSpecV1,
  fieldId: string,
): { count: number; values: string[] } | undefined {
  const field = spec.inputFields.find(item => item.id === fieldId);
  const range = field && parseTokenRange(field.encoding);
  if (!field || !range || !field.dependsOn?.includes(range.countFieldId)) return undefined;
  const countField = spec.inputFields.find(item => item.id === range.countFieldId);
  const countLocation = countField && parseLocation(countField.encoding);
  if (!countField || !countLocation || !scalarLocationIsUnambiguous(
    spec,
    countField.id,
    countLocation,
  )) return undefined;
  const count = parseNonNegativeInteger(
    tokenValuesAtLine(input, countLocation.line)?.[countLocation.token - 1],
  );
  const tokens = tokenValuesAtLine(input, range.line);
  if (count === undefined || !tokens || range.startToken > tokens.length + 1) return undefined;
...
function evaluateRecognizedSemantic(
  ...
  if (request.constructionKind === 'graph-self-loop'
    ...
    if (request.constructionKind === 'graph-self-loop'
      || request.constructionKind === 'graph-duplicate-edge') return isSimpleUndirected(layout);
    ...
    if (request.constructionKind === 'tree-missing-edge'
      || request.constructionKind === 'tree-cycle') return isTree(layout);
...
function applicableRecognizableSemantics(
  ...
  const invariants: Target[] = spec.invariants.map(invariant => ({
    id: invariant.id,
...
function constructMutationForRequest(
  input: string,
  spec: ProblemSpecV1,
  target: Target,
  request: InternalProbeRequest,
...
export function buildConstraintProbes(
  ...
  const customRequests = recipes.flatMap(recipe => {
    ...
    if (deterministicallyAttemptedTargets.has(targetKey)) return [];
  ...
  const deduplicatedProbes = probes.filter(probe => {
    if (seenProbeIds.has(probe.id)) return false;
    seenProbeIds.add(probe.id);
    return true;
  });
  const publicGapKey = (item: ConstraintProbeGap): string => canonicalJson({
