...
export interface PythonTemplateInterfaceReport {
  kind: 'ok' | 'conflict' | 'syntax';
  names?: string[];
...
function inspectionProgram(solution: string, template: string): string {
...
export function buildPythonTemplateInspection(solution: string, template: string): string {
...
export function buildPythonTemplateExecution(solution: string, template: string): string {
...
export function parsePythonTemplateInterfaceReport(raw: string): PythonTemplateInterfaceReport {
  const value = JSON.parse(raw);
  if (value?.kind === 'ok' && Object.keys(value).length === 1) return value;
  if (value?.kind === 'conflict' && Object.keys(value).length === 2 && Array.isArray(value.names)
    && value.names.length > 0 && value.names.length <= 30
    && value.names.every((name: unknown) => typeof name === 'string' && name.length > 0 && name.length <= 256)) return value;
  if (value?.kind === 'syntax' && Object.keys(value).length === 2
    && ['solution', 'template'].includes(value.section)) return value;
  throw new Error('Invalid Python template interface inspection result');
}

export function assertPythonTemplateInterface(report: PythonTemplateInterfaceReport): void {
  ...
  throw new TestdataPipelineError(
    report.kind === 'conflict'
      ...
      : `Python ${solution ? 'SOLUTION' : '模板'}语法无效，必须修复对应源码。`,
    solution ? 'SPEC_PARSE_FAILED' : 'TEMPLATE_COMPILE_FAILED',
    solution ? 'oracle' : 'template', solution ? 'oracle' : 'template-py', 'repair-artifact',
    { failureKind: report.kind === 'conflict' ? 'interface' : 'syntax' },
  );
}

export function assertPythonTemplateExecutionDiagnostic(stderr: string): void {
  const line = stderr.split('\n').find(item => item.startsWith(MARKER));
  if (!line || line.length > 12000) return;
  let report: PythonTemplateInterfaceReport;
  try { report = parsePythonTemplateInterfaceReport(line.slice(MARKER.length)); } catch { return; }
