...
/** Explicit, deterministic inputs only. This module never reads persistence or calls AI. */
export interface TestdataRiskInput {
  statement: string;
...
const CUSTOM_CHECKER_SIGNAL: RiskSignal = {
...
function tierForScore(score: number): Exclude<TestdataRiskTier, 'blocked'> {
  if (score >= 6) return 'high';
  if (score >= 3) return 'medium';
  return 'low';
}

export function getTestdataReliabilityMode(
  raw = process.env.AI_HELPER_TESTDATA_RELIABILITY_MODE,
): TestdataReliabilityMode {
  const value = String(raw || 'observe').trim().toLowerCase();
  return value === 'legacy' || value === 'enforce' ? value : 'observe';
}

export function getTestdataDirectFallbackEnabled(
  raw = process.env.AI_HELPER_TESTDATA_ALLOW_DIRECT_FALLBACK,
): boolean {
...
export function assessTestdataRisk(input: TestdataRiskInput): TestdataRiskAssessment {
  ...
  const addIf = (condition: boolean, risk: RiskSignal) => {
    if (condition) reasons.push(risk);
  };

  addIf(!!input.hasCustomChecker, CUSTOM_CHECKER_SIGNAL);
  addIf(RISK_PATTERNS.multipleValidOutput.test(statement), signal(
    'MULTIPLE_VALID_OUTPUT', 3, 'ai_helper_testdata_risk_multiple_valid_output',
  ));
  addIf(RISK_PATTERNS.floatingPoint.test(statement), signal(
    'FLOATING_POINT', 3, 'ai_helper_testdata_risk_floating_point',
  ));
  addIf(RISK_PATTERNS.statefulOperations.test(statement), signal(
    'STATEFUL_OPERATIONS', 2, 'ai_helper_testdata_risk_stateful_operations',
  ));
