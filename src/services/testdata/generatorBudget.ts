...
export const GENERATOR_BYTE_LIMITS = {
  ...
  output: TESTDATA_OUTPUT_MAX_BYTES,
  stdout: 8 * 1024 * 1024,
...
GENERATOR_BUDGET: ${JSON.stringify({
    ...
    maxOutputBytes: TESTDATA_OUTPUT_MAX_BYTES,
...
print(payload, end='')
若估算必要覆盖的最小输入或输出仍超硬上限，停止生成，仅返回 ${GENERATOR_BUDGET_CONFLICT_MARKER} 后跟严格 JSON {"scope":"input","minimumBytes":12000000}。scope 仅 input（单个 .in）、output（单个 .out，4 MiB）、stdout（整批 JSON）或 plan（全部文件）；minimumBytes 必须为超过对应硬上限的整数。服务端会将模型估算交给人工复核，不视为已经执行验证的事实。不得混入 GENERATOR、GENERATOR_PLAN、TEMPLATE 或其他分节。`;
}
...
export function assertGeneratedDataBudget(
  ...
  const oversized = byCase.flatMap((sizes, index) => sizes.flatMap((bytes, side) => {
    ...
    return bytes > GENERATOR_BYTE_LIMITS[scope] ? [{ index, scope, bytes, limit: GENERATOR_BYTE_LIMITS[scope] }] : [];
  }));
  if (oversized.length > 0) {
    const first = oversized[0];
    throw new TestdataPipelineError(
      `数据文件超限（测试点:类型=字节）：${oversized.map(x => `${x.index + 1}:${x.scope}=${x.bytes}`).join(', ')}。`
        + `全部 CASE 输入与输出字节：${indexes.join('; ')}。单个数据文件最多 4 MiB；必须联合规划整批预算，保留必要覆盖，不得截断答案。`,
      'GENERATOR_OUTPUT_TOO_LARGE', 'generator', 'generator', 'repair-artifact',
      { failureKind: `${first.scope}-budget`, caseIndex: first.index + 1, actualBytes: first.bytes, maxBytes: first.limit, indexes },
    );
  }
