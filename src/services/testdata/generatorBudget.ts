...
print(payload, end='')
若估算必要覆盖的最小输入或输出仍超硬上限，停止生成，仅返回 ${GENERATOR_BUDGET_CONFLICT_MARKER} 后跟严格 JSON {"scope":"input","minimumBytes":12000000}。scope 仅 input（单个 .in）、stdout（整批 JSON）或 plan（全部文件）；minimumBytes 必须为超过对应硬上限的整数。服务端会将模型估算交给人工复核，不视为已经执行验证的事实。不得混入 GENERATOR、GENERATOR_PLAN、TEMPLATE 或其他分节。`;
}
