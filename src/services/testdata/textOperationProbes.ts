...
import type { ConstraintProbeGap } from './constraintProbes';
import type { ValidatorProbeConstructionKind } from './validatorManifest';
import { TESTDATA_INPUT_MAX_BYTES } from './fileBudget';

type Gap = ConstraintProbeGap['reasonCode'];
type Location = { line: number; token: number };
type Field = ProblemSpecV1['inputFields'][number];
export type RangeProbeKind = 'operation-range-below-min' | 'operation-range-reversed' | 'operation-range-above-max';
export const RANGE_PROBE_KINDS: RangeProbeKind[] = [
  'operation-range-below-min', 'operation-range-reversed', 'operation-range-above-max',
];

function location(encoding: string): Location | undefined {
  const match = /^line:([1-9]\d*) token:([1-9]\d*)$/.exec(encoding);
  if (!match || !match.slice(1).every(value => Number.isSafeInteger(Number(value)))) return undefined;
...
export function operationLayout(field: Field): { start: number; countId: string } | undefined {
  if (field.type !== 'operations') return undefined;
  ...
  if (!match || !Number.isSafeInteger(Number(match[1])) || Number(match[1]) !== Number(match[3]) + 1
    || !field.dependsOn?.includes(match[2])) return undefined;
  return { start: Number(match[1]), countId: match[2] };
}

function unambiguous(spec: ProblemSpecV1, field: Field, at: Location): boolean {
  return !spec.inputFields.some(other => {
    if (other.id === field.id) return false;
    const point = location(other.encoding);
    if (point) return point.line === at.line && point.token === at.token;
    const sequence = /^line:(\d+) tokens:(\d+)\.\./.exec(other.encoding);
    if (sequence) return Number(sequence[1]) === at.line && Number(sequence[2]) <= at.token;
    const lines = /^lines:(\d+)\.\./.exec(other.encoding);
    return !!lines && at.line >= Number(lines[1]);
  });
}

export function scalarSnapshot(input: string, spec: ProblemSpecV1, field: Field): { value: string; at: Location } | undefined {
  const at = location(field.encoding);
  if (!at || !unambiguous(spec, field, at)) return undefined;
  ...
  if (value === undefined && !(field.type === 'string' && at.token === 1 && tokens.length === 0
    && !spec.inputFields.some(other => other.id !== field.id && location(other.encoding)?.line === at.line))) return undefined;
  return { value: value ?? '', at };
}

export function stringAlphabet(expression: string, fieldId: string): RegExp | undefined {
  if (expression === `characters(${fieldId}) in [01]`) return /^[01]*$/;
  if (expression.startsWith(`characters(${fieldId}[1..`) && expression.endsWith(']) in [01]')) return /^[01]*$/;
  if (expression === `characters(${fieldId}) in [a-z]`) return /^[a-z]+$/;
  return undefined;
}

export function stringCharacterSnapshot(input: string, spec: ProblemSpecV1, field: Field, expression: string): {
  value: string; at: Location; checkedLength: number; valid: boolean;
} | undefined {
  const alphabet = stringAlphabet(expression, field.id);
  const snapshot = scalarSnapshot(input, spec, field);
  if (!alphabet || !snapshot) return undefined;
  ...
  if (expression.startsWith(prefix)) {
    ...
    if (typeof raw !== 'string' || !/^(0|[1-9]\d*)$/.test(raw) || !Number.isSafeInteger(Number(raw))) return undefined;
...
export function stringCountField(spec: ProblemSpecV1, field: Field, expression: string): Field | undefined {
...
export function stringLengthIsValid(input: string, spec: ProblemSpecV1, field: Field, expression: string): boolean | undefined {
  ...
  if (raw === undefined || !/^(0|[1-9]\d*)$/.test(raw) || !Number.isSafeInteger(Number(raw))
    || value === undefined || !/^[\x21-\x7e]*$/.test(value)) return undefined;
  return value.length === Number(raw);
}

export function replaceScalar(input: string, at: Location, replacement: string): { input: string; position: Location } | undefined {
  const lines = input.split('\n');
  const line = lines[at.line - 1];
  if (line === undefined) return undefined;
  const token = [...line.matchAll(/\S+/g)][at.token - 1];
  if (!token && !(at.token === 1 && !line.trim())) return undefined;
...
export function operationArgumentBounds(spec: ProblemSpecV1,
  operation: NonNullable<ProblemSpecV1['operations']>[number], expression: string) {
  const match = /^(-?\d+) <= ([A-Za-z][A-Za-z0-9_.:-]{0,63}) <= (-?\d+)$/.exec(expression);
  if (!match) return undefined;
  const min = Number(match[1]); const max = Number(match[3]);
  ...
  if (!Number.isSafeInteger(min) || !Number.isSafeInteger(max) || min > max || index < 0
    || !spec.inputFields.some(field => field.id === match[2] && field.type === 'integer'
      && field.encoding === `operation-argument:${field.id}`)) return undefined;
  return { min, max, index };
}

export function rangeDescriptor(spec: ProblemSpecV1, expression: string, fieldId: string): RangeDescriptor | undefined {
  const fields = spec.inputFields.filter(field => field.type === 'operations');
  const field = fields.find(item => item.id === fieldId);
  if (!field || fields.length !== 1) return undefined;
  ...
  if (!upperField || !location(upperField.encoding)) return undefined;
  ...
  if (!definitions.length || new Set(definitions.map(item => item.name)).size !== definitions.length
    || definitions.some(item => !/^(?:[A-Za-z][A-Za-z0-9_]*|0|[1-9]\d*)$/.test(item.name)
      ...
      || item.preconditions.some(predicate => ![0, 1].some(lower => (
        predicate === `${lower} <= ${match[2]} <= ${match[3]} <= ${match[4]}`
      )) && !operationArgumentBounds(spec, item, predicate))
      ...
      || item.arguments.slice(2).some(id => !spec.inputFields.some(argument => argument.id === id
        && argument.type === 'integer' && argument.encoding === `operation-argument:${id}`)))) return undefined;
  return { field, lower: Number(match[1]), left: match[2], right: match[3], upperField };
}

export function rangeSnapshot(input: string, spec: ProblemSpecV1, descriptor: RangeDescriptor): {
  upper: number; count: number; start: number; countId: string;
  operations: Array<{ name: string; left: number; right: number; arguments: number[]; line: number }>;
} | undefined {
  ...
  if (!countValue || countValue.at.line >= layout.start || !upperValue || upperValue.at.line >= layout.start
    || !/^(0|[1-9]\d*)$/.test(countValue.value) || !/^(0|[1-9]\d*)$/.test(upperValue.value)) return undefined;
  // Another field cannot own any part of the operation rows.
  if (spec.inputFields.some(field => {
    if (field.id === descriptor.field.id) return false;
    ...
    if (field.type === 'array') {
      ...
      if (!sequence || !Number.isSafeInteger(Number(sequence[1])) || Number(sequence[1]) >= layout.start
        || !field.dependsOn?.includes(sequence[2])) return true;
      ...
      if (!length || length.at.line >= Number(sequence[1]) || !/^(0|[1-9]\d*)$/.test(length.value)
        || !Number.isSafeInteger(Number(length.value))) return true;
  ...
  if (!Number.isSafeInteger(count) || !Number.isSafeInteger(upper)) return undefined;
  const lines = input.endsWith('\n') ? input.slice(0, -1).split('\n') : input.split('\n');
  if (lines.length !== layout.start - 1 + count) return undefined;
  ...
  for (let index = layout.start - 1; index < lines.length; index++) {
    ...
    if (!definition || tokens.length !== definition.arguments.length + 1 || !tokens.slice(1).every(raw => /^-?(0|[1-9]\d*)$/.test(raw)
      && Number.isSafeInteger(Number(raw)))) return undefined;
...
export function rangeIsValid(input: string, spec: ProblemSpecV1, descriptor: RangeDescriptor): boolean | undefined {
...
export function constructRangeMutation(input: string, spec: ProblemSpecV1, descriptor: RangeDescriptor,
  kind: ValidatorProbeConstructionKind, operationName?: string): { input: string; position: Location } | Gap {
  const snapshot = rangeSnapshot(input, spec, descriptor);
  if (!snapshot || rangeIsValid(input, spec, descriptor) !== true) return 'MUTATION_NOT_ISOLATED';
  const op = snapshot.operations.find(item => item.name === operationName);
  if (!op) return 'NO_MATCHING_LEGAL_SEED';
  let left = op.left;
  let right = op.right;
  if (kind === 'operation-range-below-min') left = descriptor.lower - 1;
  else if (kind === 'operation-range-above-max') right = snapshot.upper + 1;
  else if (kind === 'operation-range-reversed') {
    if (snapshot.upper <= descriptor.lower) return 'MUTATION_NOT_ISOLATED';
    left = descriptor.lower + 1;
    right = descriptor.lower;
  } else return 'INVALID_RECIPE';
  if (!Number.isSafeInteger(left) || !Number.isSafeInteger(right)) return 'UNSUPPORTED_TARGET';
...
export function preserveTextOperationCounts(original: string, input: string, spec: ProblemSpecV1,
  expressions: string[], countId: string, count: number): string | { gap: Gap } {
  for (const field of spec.inputFields) {
    if (field.type === 'string' && expressions.includes(`length(${field.id}) = ${countId}`)) {
      ...
      if (!snapshot || stringLengthIsValid(original, spec, field, expression) !== true || count < 0) return { gap: 'MUTATION_NOT_ISOLATED' };
      if (count > TESTDATA_INPUT_MAX_BYTES) return { gap: 'PROBE_TOO_LARGE' };
      const value = snapshot.value;
      if (Buffer.byteLength(input, 'utf8') + count - value.length > TESTDATA_INPUT_MAX_BYTES) return { gap: 'PROBE_TOO_LARGE' };
      if (!value.length && count) return { gap: 'MUTATION_NOT_ISOLATED' };
      ...
      if (!changed) return { gap: 'MUTATION_NOT_ISOLATED' };
      input = changed.input;
    }
    if (field.type !== 'operations') continue;
    const encodedCount = /^lines:[1-9]\d*\.\.([A-Za-z][A-Za-z0-9_.:-]{0,63})\+/.exec(field.encoding)?.[1];
    if (encodedCount ? encodedCount !== countId : !field.dependsOn?.includes(countId)) continue;
    const predicates = [...expressions, ...(spec.operations || []).flatMap(operation => (
      operation.preconditions.map(predicate => `for every operation, ${predicate}`)
    ))];
    const descriptor = predicates.map(expression => rangeDescriptor(spec, expression, field.id)).find(Boolean);
    if (!descriptor || operationLayout(field)?.countId !== countId) return { gap: 'MUTATION_NOT_ISOLATED' };
    const snapshot = rangeSnapshot(original, spec, descriptor);
    if (!snapshot || count < 0) return { gap: 'MUTATION_NOT_ISOLATED' };
    const lines = input.endsWith('\n') ? input.slice(0, -1).split('\n') : input.split('\n');
    const prefix = lines.slice(0, snapshot.start - 1).join('\n') + '\n';
    const rows = lines.slice(snapshot.start - 1);
    if (!rows.length && count) return { gap: 'MUTATION_NOT_ISOLATED' };
    const extra = Math.max(0, count - rows.length);
    if (extra * (Buffer.byteLength(rows[0] || '', 'utf8') + 1) + Buffer.byteLength(input, 'utf8') > TESTDATA_INPUT_MAX_BYTES) return { gap: 'PROBE_TOO_LARGE' };
    input = prefix + rows.slice(0, count).map(row => row + '\n').join('') + (rows[0] + '\n').repeat(extra);
  }
  return Buffer.byteLength(input, 'utf8') > TESTDATA_INPUT_MAX_BYTES ? { gap: 'PROBE_TOO_LARGE' } : input;
}
