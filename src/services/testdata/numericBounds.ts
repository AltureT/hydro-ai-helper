...
export type NumericBoundParseResult =
  | { kind: 'parsed'; fieldId: string; bounds: NumericBounds }
  | { kind: 'not-bound' }
  | { kind: 'invalid-bound' };
...
function boundedExponent(raw: string): number | undefined {
...
export function canonicalIntegerLiteral(raw: string): string | undefined {
  if (raw.length === 0 || raw.length > MAX_LITERAL_LENGTH) return undefined;
  if (/^(?:0|-?[1-9]\d*)$/.test(raw)) {
    const digits = raw.startsWith('-') ? raw.length - 1 : raw.length;
    if (digits > MAX_LITERAL_DIGITS) return undefined;
    try {
  ...
  if (scientific) {
    ...
    if (exponent === undefined || scientific[2].length > MAX_LITERAL_DIGITS) return undefined;
...
export function compareIntegerLiterals(left: string, right: string): number {
  const leftValue = BigInt(left);
  const rightValue = BigInt(right);
  return leftValue < rightValue ? -1 : leftValue > rightValue ? 1 : 0;
}

export function safeNumberFromIntegerLiteral(value: string | undefined): number | undefined {
...
export function parseNumericBoundExpression(expression: string): NumericBoundParseResult {
  ...
  const range = new RegExp(
    `^${INTEGER_TOKEN}\\s*<=\\s*${FIELD_REFERENCE}\\s*<=\\s*${INTEGER_TOKEN}$`,
  ).exec(normalized);
  if (range) {
  ...
  const fieldBound = new RegExp(
    `^${FIELD_REFERENCE}\\s*(<=|>=)\\s*${INTEGER_TOKEN}$`,
  ).exec(normalized);
  if (fieldBound) {
    const literal = canonicalIntegerLiteral(fieldBound[3]);
    if (literal === undefined) return { kind: 'invalid-bound' };
    return {
      kind: 'parsed',
  ...
  const reversedBound = new RegExp(
    `^${INTEGER_TOKEN}\\s*(<=|>=)\\s*${FIELD_REFERENCE}$`,
  ).exec(normalized);
  if (reversedBound) {
