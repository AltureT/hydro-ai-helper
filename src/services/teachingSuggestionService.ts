...
- 挖空指令以 Markdown 表格格式输出（见下方 fill_in_exercise 章节格式）
...
> 💡 学生拿到的代码中，上述位置会被替换为空白，需要自行填写正确代码。`;
...
export function buildMainPrompt(input: MainPromptInput): PromptMessages {
  ...
  if (hasBehavior) outputSections.push('p2_behavior_intervention');
  if (hasFillIn) outputSections.push('fill_in_exercise');
