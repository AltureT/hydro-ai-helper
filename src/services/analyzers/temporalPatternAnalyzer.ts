...
const BURST_INTERVAL_MS = 60_000;          // 60s
const BURST_MIN_COUNT = 3;
const SESSION_GAP_MS = 30 * 60_000;       // 30 min
 ...
 */
export function countStatusTransitions(records: TimedRecord[]): number {
...
export function analyzeTemporalPatterns(
  ...
  pids: number[],
  studentUids: number[],
  conversationsByUserPid: Map<string, boolean>,
  contestStartTime?: Date,
  contestEndTime?: Date,
  outProfiles?: StudentTemporalProfile[],
  ...
  const disengagedThreshold = Math.max(2 * 60 * 60_000, contestDurationMs * 0.5);

  // Group records by uid:pid
  const recordsByUidPid = new Map<string, TimedRecord[]>();
  ...
  for (const uid of studentUids) {
    ...
    for (const pid of pids) {
      ...
      if (!pidRecords || pidRecords.length === 0) continue;
  ...
  const actionablePatterns: TemporalPatternLabel[] = [
    'stuck_silent',
    'burst_then_quit',
    'disengaged',
    'persistent_learner',
  ...
  for (const pattern of actionablePatterns) {
    ...
    const severity: TeachingFinding['severity'] =
      pattern === 'stuck_silent' || pattern === 'burst_then_quit' ? 'high' : 'medium';
