...
export interface FillInTriggerInput {
  hasCommonError: boolean;
  finalACRate: number;
  firstAttemptACRate: number;
 ...
 */
export function shouldGenerateFillIn(input: FillInTriggerInput): boolean {
  if (!input.hasCommonError) return false;
  if (input.finalACRate >= 0.9 && input.avgSubmissionCount < 2) return false;
  if (input.firstAttemptACRate > 0.7) return false;
  return true;
}
...
export function isFillInBlankProblem(problemContent: string): boolean {
 ...
 */
export function scoreReadability(code: string, medianLines?: number): number {
  let score = 0;
  const lines = code.split('\n');

  if (/\/\/|\/\*|#\s|'''|"""/m.test(code)) score++;

  if (/\b(function|def|int\s+\w+\s*\(|void\s+\w+\s*\(|class\s+|public\s+)/m.test(code)) score++;
...
export function selectACCode(
  ...
  const scored: ScoredSubmission[] = filtered.map(s => ({
    uid: s.uid,
    code: s.code,
    lang: s.lang,
