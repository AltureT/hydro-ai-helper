...
function sanitizeForPrompt(text) {
    ...
    result = result.replace(SAFETY_TAG_PATTERN, (match) => match.replace(/</g, '\uFF1C').replace(/>/g, '\uFF1E'));
    result = result.replace(BOUNDARY_MARKER_PATTERN, (match) => match.replace(/---/g, '\uFF0D\uFF0D\uFF0D'));
    result = result.replace(ROLE_TAG_PATTERN, (match) => match.replace(/\[/g, '\uFF3B').replace(/\]/g, '\uFF3D'));
    result = result.replace(TRIPLE_BACKTICK, (match) => match.split('').map((c, i) => (i > 0 && i % 2 === 0) ? '\u2006' + c : c).join(''));
    return result;
}
