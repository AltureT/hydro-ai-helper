...
function buildNumericPrefixRanges(query) {
    if (!/^\d+$/.test(query) || (query.length > 1 && query.startsWith('0')))
        return [];
    const prefix = Number(query);
    if (!Number.isSafeInteger(prefix) || prefix < 0)
        return [];
    if (prefix === 0)
        return [{ userId: { $gte: 0, $lt: 1 } }];
    const ranges = [];
...
class JailbreakLogModel {
    ...
    async suggestUserIds(domainId, query, limit = 10) {
    ...
    async suggestProblemIds(domainId, query, limit = 10) {
        const normalizedQuery = query.trim().slice(0, MAX_FILTER_SUGGESTION_QUERY_LENGTH);
        if (!normalizedQuery)
            return [];
        const safeLimit = Math.min(20, Math.max(1, Math.floor(limit)));
