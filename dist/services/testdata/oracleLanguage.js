...
function languageError(message) {
    throw new failures_1.TestdataPipelineError(message, 'SPEC_PARSE_FAILED', 'oracle', 'oracle', 'repair-artifact', { failureKind: 'language' });
}
/** Recognize only a C++ opening statement, never keywords inside Python strings/comments. */
function hasCppOpening(source) {
...
function parseOracleLanguage(raw, problemType, source = '', fallback = 'python') {
    const text = raw.replace(/<think>[\s\S]*?<\/think>/g, '');
    const declarations = [...text.matchAll(/(?:^|\r?\n)[ \t]*(?:@@@ORACLE_LANG@@@|===\s*ORACLE_LANG\s*===)[ \t]*(?:\r?\n|$)(?:[ \t]*\r?\n)*([^\r\n]*)/gi)].map(match => {
        const value = match[1].trim().toLowerCase();
        if (['cpp', 'c++', 'cpp17', 'c++17'].includes(value))
            return 'cpp';
        if (['python', 'python3', 'python 3'].includes(value))
            return 'python';
        return languageError('ORACLE_LANG 必须为 python 或 cpp，不能把缺损或未知声明按 Python 执行。');
    });
    if (new Set(declarations).size > 1)
        languageError('ORACLE_LANG 存在互相冲突的声明。');
...
//# sourceMappingURL=oracleLanguage.js.map