...
exports.deriveCoverageTargets = deriveCoverageTargets;
exports.computeCoverageFeatures = computeCoverageFeatures;
exports.evaluateSemanticCoverage = evaluateSemanticCoverage;
exports.enforceCoverageRequirements = enforceCoverageRequirements;
...
const MAX_TOTAL_FEATURE_WORK = 1000000;
function boundsByField(spec, scope) {
    ...
    for (const constraint of spec.constraints) {
        ...
        if (parsed.kind === 'invalid-bound')
            throw new Error('invalid numeric bound');
        if (parsed.kind === 'not-bound')
            continue;
        const merged = (0, numericBounds_1.intersectIntegerBounds)(result.get(parsed.fieldId) || {}, parsed.bounds);
        if (!merged)
            throw new Error('incompatible numeric bounds');
        result.set(parsed.fieldId, merged);
    }
    return result;
}
function sizeFieldIds(spec) {
...
function addBoundaryTargets(targets, fieldId, bounds, size, subtaskId) {
    const scope = subtaskId === undefined ? {} : { subtaskId };
    const minNumber = (0, numericBounds_1.safeNumberFromIntegerLiteral)(bounds.min);
    const maxNumber = (0, numericBounds_1.safeNumberFromIntegerLiteral)(bounds.max);
...
function hasInvariant(spec, fieldId, kind) {
...
function deriveCoverageTargets(spec) {
    const targets = [];
    ...
    for (const subtask of spec.subtasks) {
        ...
        for (const [fieldId, fieldBounds] of scopedBounds) {
            const effectiveBounds = (0, numericBounds_1.intersectIntegerBounds)(bounds.get(fieldId) || {}, fieldBounds);
            if (!effectiveBounds)
                throw new Error('incompatible scoped numeric bounds');
            addBoundaryTargets(targets, fieldId, effectiveBounds, sizes.has(fieldId), subtask.id);
        }
    }
    for (const field of spec.inputFields) {
...
function appendNumericValues(target, fieldId, values) {
...
function appendIntegerLiterals(target, fieldId, values) {
    const literals = values.map(value => (0, numericBounds_1.canonicalIntegerLiteral)(String(value)));
...
function sequenceCharacteristics(values) {
    if (values.length === 0)
...
function adjacencyFor(vertexCount, edges) {
    if (!Number.isSafeInteger(vertexCount) || vertexCount < 1)
        throw new Error('invalid vertices');
    const adjacency = Array.from({ length: vertexCount + 1 }, () => []);
    const keys = new Set();
...
function graphMetrics(vertexCount, edges) {
    ...
    for (const neighbours of adjacency)
        maxDegree = Math.max(maxDegree, neighbours.length);
    let depthFromOne = 0;
    for (const value of distance)
        depthFromOne = Math.max(depthFromOne, value);
    return {
        components,
        hasCycle,
        hasBridge,
...
function classifyTreeShape(vertexCount, edges, metrics) {
    if (metrics.maxDegree === vertexCount - 1)
        return 'star';
    if (metrics.maxDegree <= 2)
        return 'chain';
    ...
    const actualKeys = new Set(edges.map(([left, right]) => (left < right ? `${left}:${right}` : `${right}:${left}`)));
    if (balancedKeys.size === actualKeys.size
        && [...balancedKeys].every(key => actualKeys.has(key)))
        return 'balanced';
    const adjacency = adjacencyFor(vertexCount, edges);
    ...
    if (highDegreeCount === 1 && pathVertexCount > 0)
        return 'broom';
    return 'random';
}
function operationFeatures(operations) {
    ...
    for (let index = operations.length - 1; index >= 0; index -= 1) {
        ...
        if (['ADD', 'DEL', 'DELETE'].includes(normalizedTypes[index]))
            laterUpdate = true;
    }
    const activeAdds = new Map();
    const removalByAddIndex = Array(operations.length);
    let updateCount = 0;
    let addDeleteRepeat = false;
    const completedLifetimeKeys = new Set();
    let queryBetweenUpdates = false;
    operations.forEach((operation, index) => {
        const type = normalizedTypes[index];
        increment(types, type);
        if (index > 0)
            increment(transitions, `${normalizedTypes[index - 1]}->${type}`);
        const key = operation.arguments[0];
        if (type === 'ADD') {
            updateCount += 1;
            if (completedLifetimeKeys.has(key))
                addDeleteRepeat = true;
            if (activeAdds.has(key))
                throw new Error('duplicate active key');
            ...
            if (addIndex === undefined)
                throw new Error('delete without active key');
            ...
            if (updateCount > 0 && hasUpdateAfter[index])
                queryBetweenUpdates = true;
        }
    });
    let maxRemoval = -1;
...
function computeCoverageFeatures(cases, spec) {
    if (!Array.isArray(cases) || cases.length === 0)
        throw new Error('no structured cases');
    let totalWork = 0;
    for (const caseValue of cases) {
        if (!caseValue || typeof caseValue !== 'object'
            || !caseValue.values || typeof caseValue.values !== 'object')
            throw new Error('case values');
        for (const rawValue of Object.values(caseValue.values)) {
            ...
            if (value.kind === 'integer')
                ...
                totalWork += value.operations.length;
            else
    ...
    const operationArgumentFields = spec?.inputFields.filter(field => (field.type === 'integer' && field.encoding === `operation-argument:${field.id}`)) || [];
    ...
    for (const caseValue of cases) {
        ...
        if (caseValue.subtaskId !== undefined)
            subtaskIds.add(caseValue.subtaskId);
        for (const [fieldId, rawValue] of Object.entries(caseValue.values)) {
            ...
            if (value.kind === 'integer') {
                ...
                if (sequence.ordered)
                    features.orderedCount += 1;
                if (sequence.reversed)
                    features.reversedCount += 1;
                if (sequence.allEqual)
                    features.allEqualCount += 1;
                if (sequence.alternating)
                    features.alternatingCount += 1;
            }
            else if (value.kind === 'matrix') {
                if (!Array.isArray(value.values) || value.values.length === 0
                    || value.values.some(row => !Array.isArray(row) || row.length === 0
                        || row.length !== value.values[0].length))
                ...
                increment(features.treeShapeCounts, classifyTreeShape(value.vertexCount, value.edges, metrics));
            }
                ...
                if (value.edges.length <= value.vertexCount - 1)
                    features.graphShapeCounts.sparse += 1;
                ...
                if (density >= 0.75)
                    features.graphShapeCounts.dense += 1;
            }
            else if (value.kind === 'operation-sequence') {
                if (operationArgumentFieldId !== undefined) {
                    ...
                    appendNumericValues(features.integerValuesByField, operationArgumentFieldId, serializedArguments);
...
function targetActual(target, features) {
...
function evaluateSemanticCoverage(input) {
    ...
    try {
        ...
        const scopedFeatures = new Map();
        const featuresForTarget = (target) => {
            if (target.subtaskId === undefined)
                return features;
            if (scopedFeatures.has(target.subtaskId))
                return scopedFeatures.get(target.subtaskId);
        ...
        return {
            ...
            passedTargets: matrix.reduce((total, row) => total + Math.min(row.actual, row.required), 0),
            criticalMissing: matrix.reduce((total, row) => (total + (row.critical ? Math.max(0, row.required - row.actual) : 0)), 0),
        };
    }
    catch {
        return {
...
function enforceCoverageRequirements(coverage, riskTier, reliabilityMode) {
    if (coverage.mode !== 'trusted-dsl'
        || coverage.criticalMissing === 0
        || riskTier !== 'high'
        || reliabilityMode !== 'enforce')
        return;
    throw new failures_1.TestdataPipelineError('高风险题目的受信生成器计划缺少关键语义覆盖目标。', 'COVERAGE_REQUIREMENT_MISSING', 'generator', 'coverage', 'repair-artifact', { missingCount: coverage.criticalMissing });
}
