 ...
 */
function correlateErrorAI(errorFinding, aiUserUids, recordsByPidUid, totalStudents) {
    ...
    for (const uid of affectedStudents) {
        ...
        if (!records.some(r => r.status === 1) && !submissionEvidence_1.ERROR_STATUSES.has(records[records.length - 1]?.status))
            continue;
    ...
    function acRate(uids) {
        let acCount = 0;
        for (const uid of uids) {
            const key = `${pid}:${uid}`;
            const records = recordsByPidUid.get(key) ?? [];
            if (records.some(r => r.status === 1))
                acCount++;
        }
    ...
    const statusLabels = { 2: 'WA', 3: 'TLE', 4: 'MLE', 5: 'OLE', 6: 'RE', 7: 'CE' };
 ...
 */
function correlateAtRiskTemporal(atRiskUids, temporalProfiles, totalStudents) {
    if (atRiskUids.length < MIN_GROUP_SIZE)
        return null;
    const atRiskSet = new Set(atRiskUids);
    // For each at-risk student, find their worst pattern across all profiles
    const studentWorstPattern = new Map();
    for (const profile of temporalProfiles) {
        if (!atRiskSet.has(profile.uid))
            continue;
    ...
    const breakdownParts = Object.entries(patternCounts)
        .sort((a, b) => b[1] - a[1])
 ...
 */
function correlateDifficultyError(difficultyFinding, errorClusterFindings, totalStudents) {
    ...
    const matchingClusters = errorClusterFindings.filter(f => f.evidence.affectedProblems.some(p => problemSet.has(p)));
    if (matchingClusters.length === 0)
        return null;
    // Pick the largest cluster (most affected students)
    const largestCluster = matchingClusters.reduce((best, current) => current.evidence.affectedStudents.length > best.evidence.affectedStudents.length
        ? current
        : best);
    const passRate = difficultyFinding.evidence.metrics.passRate
        ?? Math.round(((difficultyFinding.evidence.metrics.accepted ?? 0) /
...
function analyzeCorrelations(findings, temporalProfiles, totalStudents, aiUserUids, recordsByPidUid) {
    ...
    for (const atRiskFinding of atRiskFindings) {
        const result = correlateAtRiskTemporal(atRiskFinding.evidence.affectedStudents, temporalProfiles, totalStudents);
        if (result)
            results.push({ ...result, sourceFindingId: atRiskFinding.id });
    }
