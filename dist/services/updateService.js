        ...
        try {
            ...
            setTimeout(async () => {
                try {
                    ...
                    // 尝试 pm2 reload（零停机）
                    const reloadResult = await this.executeCommand(pm2Path, ['reload', 'hydrooj'], this.pluginPath, undefined, 30000 // 30秒超时
                    );
                    if (reloadResult.code !== 0) {
                        // reload 失败，降级为 restart
                        console.log('[UpdateService] pm2 reload 失败，降级为 restart');
                        await this.executeCommand(pm2Path, ['restart', 'hydrooj'], this.pluginPath, undefined, 30000);
                    }
                }
                catch (err) {
