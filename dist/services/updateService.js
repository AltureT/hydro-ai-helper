...
class UpdateService {
     ...
     * 初始化 git 仓库并拉取代码
     */
    async initGitRepo(repoUrl, onLog) {
        const log = (msg) => onLog?.(msg);
        ...
        log('正在获取远程代码...');
        const fetchResult = await this.executeCommand('git', ['fetch', '--prune', 'origin', 'main'], this.pluginPath, (line) => log(line.trim()), 300000 // 🔒 5 分钟超时
        );
    ...
    async performUpdate(onProgress) {
        ...
        try {
            ...
            if (validation.needsGitInit) {
                ...
                const fetchResult = await this.executeCommand('git', ['fetch', '--prune', 'origin', 'main'], this.pluginPath, (line) => log('pulling', line.trim()), 300000 // 🔒 5 分钟超时
                );
                if (fetchResult.code !== 0) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    const restartResult = await this.executeCommand(pm2Path, ['restart', 'hydrooj'], this.pluginPath, undefined, 30000 // 30秒超时
                    );
                    if (restartResult.code !== 0) {
                        console.error('[UpdateService] pm2 restart 失败:', (restartResult.stderr || restartResult.stdout || '').trim());
                    }
                }
                catch (err) {
