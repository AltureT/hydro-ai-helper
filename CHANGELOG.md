...
- **测试数据区分度验证（kill-list）**：生成完成后自动构造典型错误解（边界错误、假算法、模拟溢出）与暴力复杂度检查，在沙箱中验证数据能否卡住它们；卡不住的错误解会触发最多两轮定向补刀（hack 测试点）。区分度验证完全非阻断——任何失败只降级为警告，不影响已通过正确性验证的数据。O(1)/纯模拟等无复杂度差异的题目会自动跳过暴力检查，不再误报"数据规模不足"。
- **C++ 标程支持（传统题）**：教师提供的 C++ 标准程序现可直接编译并在沙箱中实跑生成 `.out`；AI 也会在判断 Python 性能不足时自动改写 C++ 标程。大数据量、紧时限的题目从此可以生成可靠数据。编译能力不可用时自动回退原有行为。
- **自定义 checker（testlib）实跑**：题目配置 testlib checker 时，样例回归、压力对拍、区分度判定与补刀验证改用题目自带 checker 判定，SPJ 题不再整体跳过验证；checker 编译失败或运行异常时自动退回原有跳过语义并注明。
- **按题面子任务表生成分层数据**：题面含子任务/分数表时（且题目尚未配置 subtasks、测试点数量足够），自动按各档约束生成分层测试点并输出对应的 `config.yaml` 多子任务配置；面板覆盖列表按"子任务 K"展示归属。已配置 subtasks 的题目会原样保留既有配置，不再被覆盖。
- **中断任务断点恢复**：生成过程中服务重启或连接中断后，可从断点恢复——已完成的 AI 阶段产物不再重新调用模型付费，沙箱验证仍完整重跑。
- **更快的生成与修复**：定向修复后只重跑受影响的沙箱阶段（此前一律全量重跑）；输入校验与标程批跑改为并发执行；解题、生成器与独立验证器三路 AI 调用并行发起。
- **内部压力数据校验容错**：60 组内部对拍数据中个别违反题面约束时自动剔除（存活 ≥75% 即继续），不再因 1/60 组内部数据违规导致整次生成失败——高难度复杂约束题目的成功率显著提升。
- **结果说明与进度体验重构**：生成结果拆分为"需要注意的警告（置顶）/ 系统验证事实 / AI 生成说明（默认折叠）"三层；进度面板文案去术语化；定向修复返工时步骤条以 ↻ 标注待重验步骤并说明原因，不再表现为进度倒退。
- **防御性修复**：题目已有 `config.yaml` 无法解析时立即中止生成（避免覆盖丢失配置）；错误解与补刀相关的模型调用纳入 token 用量统计。
- **移除管理端"测试数据生成能力检查"面板**及其安装后引导（真实生成已自带逐项机器验证，该功能冗余且产生额外费用）。
- 升级后请重启 Hydro 以加载新后端代码并重建前端资源；无需新增配置或数据库迁移。C++ 标程与 checker 实跑依赖沙箱镜像内的 g++，不可用时相关功能自动降级。
