...
export const LOCALE_FALLBACK: Record<'zh' | 'en', Record<string, string>> = {
  "zh": {
    ...
    "ai_helper_admin_testdata_example_independence": "独立复核应与题意主解析使用不同模型，独立验证也应与标准答案使用不同模型，双方的备选链应避免重叠。换端点调用同一模型不能提供独立性；若 Flash 的复核能力不足，可换用其他模型。是否通过验证仍以生成结果为准。",
    "ai_helper_admin_testdata_example_intro": "试配思路：Pro 承担题意解析、标准答案与冲突裁决，Flash 尝试生成配套文件及复核。此组合尚未通过本插件的完整质量验证，请用有已知答案的代表题目比较正确性、耗时与修复次数后再决定。",
    "ai_helper_admin_testdata_example_models": "示例模型（按调用顺序）",
    "ai_helper_admin_testdata_example_setup": "先在端点中获取并保存模型列表，再在下方各阶段选择实际可用的模型 ID。生成器一行的 Flash → Pro 表示优先尝试 Flash，失败时回退到 Pro。示例仅供对照，不会自动填写配置。",
    "ai_helper_admin_testdata_example_stage": "阶段与选型重点",
    "ai_helper_admin_testdata_example_title": "DeepSeek Flash / Pro 分工示例",
    ...
    "ai_helper_admin_testdata_role_spec_critic_purpose": "独立读题并提取约束，供系统与主解析交叉比较。",
    ...
    "ai_helper_admin_testdata_role_verifier_purpose": "编写暴力解、小数据生成器与输入校验器，需要发现错误算法及边界问题。",
    "ai_helper_admin_testdata_roles_desc": "各阶段默认跟随“测试数据生成”的模型设置。点开某一阶段可单独修改，模型按顺序调用。",
    ...
    "ai_helper_testdata_discard_reason_checker_problem": "Checker 问题",
    "ai_helper_testdata_discard_reason_invalid_input": "输入无效",
    "ai_helper_testdata_discard_reason_label": "可选原因",
    "ai_helper_testdata_discard_reason_none": "不填写",
    "ai_helper_testdata_discard_reason_other": "其他",
    "ai_helper_testdata_discard_reason_template_problem": "模板问题",
    "ai_helper_testdata_discard_reason_weak_coverage": "覆盖不足",
    "ai_helper_testdata_discard_reason_wrong_answer": "答案不正确",
    ...
    "ai_helper_testdata_strong_model_notice": "本功能对模型的代码推理能力非常敏感。系统会按同一流程持续生成，直到完成、服务明确失败或您主动取消；持续等待可能增加付费模型费用。建议在 AI 助手配置 →「场景模型」中为「测试数据生成」指定思考能力更深、稳定性更高的模型。",
  ...
  "en": {
    ...
    "ai_helper_admin_testdata_example_independence": "Use different models for Spec Primary and Spec Critic, and for Oracle and Verifier; avoid overlap in their fallback chains. Calling the same model through another endpoint does not provide independence. If Flash cannot review reliably, choose another model. The generation result determines whether validation passed.",
    "ai_helper_admin_testdata_example_intro": "Trial setup: Pro handles the primary spec, oracle and adjudication; try Flash for supporting files and reviews. This combination has not passed full quality validation in this plugin. Compare correctness, elapsed time and repair counts on representative problems with known answers before adopting it.",
    "ai_helper_admin_testdata_example_models": "Example models (in call order)",
    "ai_helper_admin_testdata_example_setup": "Fetch and save the endpoint's model list first, then select the available model IDs in the stages below. Flash → Pro for artifacts means trying Flash first and falling back to Pro on failure. This example is for reference and does not fill in your configuration.",
    "ai_helper_admin_testdata_example_stage": "Stage and selection criteria",
    "ai_helper_admin_testdata_example_title": "DeepSeek Flash / Pro role example",
    ...
    "ai_helper_admin_testdata_role_spec_critic_purpose": "Independently extract constraints from the statement for the system to cross-check against the primary spec.",
    ...
    "ai_helper_admin_testdata_role_verifier_purpose": "Write a brute-force solution, small-input generator and input validator; requires detecting incorrect algorithms and boundary failures.",
    "ai_helper_admin_testdata_roles_desc": "Stages follow the Test Data Generation models by default. Open a stage to override its models in fallback order.",
    ...
    "ai_helper_testdata_discard_reason_checker_problem": "Checker problem",
    "ai_helper_testdata_discard_reason_invalid_input": "Invalid input",
    "ai_helper_testdata_discard_reason_label": "Optional reason",
    "ai_helper_testdata_discard_reason_none": "No reason",
    "ai_helper_testdata_discard_reason_other": "Other",
    "ai_helper_testdata_discard_reason_template_problem": "Template problem",
    "ai_helper_testdata_discard_reason_weak_coverage": "Weak coverage",
    "ai_helper_testdata_discard_reason_wrong_answer": "Incorrect answer",
    ...
    "ai_helper_testdata_strong_model_notice": "This feature is highly sensitive to code-reasoning capability. One adaptive pipeline keeps running until it completes, the service explicitly fails, or you cancel it; continued waiting may increase paid-model costs. In AI Helper settings → Scenario Models, assign a deeper-reasoning, more reliable model to Test Data Generation.",
