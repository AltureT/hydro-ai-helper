...
export const LOCALE_FALLBACK: Record<'zh' | 'en', Record<string, string>> = {
  "zh": {
    ...
    "ai_helper_testdata_profile_hard": "高难题（最长约 90 分钟）",
    "ai_helper_testdata_profile_hard_hint": "适合需要长时间算法推导和多轮验证的竞赛题：单次模型调用最多等待 30 分钟，整次后台任务最多约 90 分钟。",
    "ai_helper_testdata_profile_hard_warning": "高难题模式可能产生较高的付费模型费用；任务可在后台继续，您可以离开页面后再回来查看，但生成结果仍需人工复核。",
  ...
  "en": {
    ...
    "ai_helper_testdata_profile_hard": "Hard problem (up to about 90 min)",
    "ai_helper_testdata_profile_hard_hint": "For contest problems requiring long reasoning and multiple verification stages: up to 30 minutes per model call and about 90 minutes for the full background job.",
    "ai_helper_testdata_profile_hard_warning": "Hard-problem mode may incur substantial paid-model costs. The job continues in the background, so you can leave and return later, but its result still requires manual review.",
