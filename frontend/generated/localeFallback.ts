...
export const LOCALE_FALLBACK: Record<'zh' | 'en', Record<string, string>> = {
  "zh": {
    ...
    "ai_helper_testdata_progress_rework_notice": "验证时发现问题，正在返工修复；标记 ↻ 的步骤会在修复后重新执行。",
    ...
    "ai_helper_testdata_strong_model_notice": "⚠️ 本功能对模型的代码推理能力非常敏感。系统会按同一流程持续生成，直到完成、服务明确失败或您主动取消；持续等待可能增加付费模型费用。建议在 AI 助手配置 →「场景模型」中为「测试数据生成」指定思考能力更深、稳定性更高的模型。",
  ...
  "en": {
    ...
    "ai_helper_testdata_progress_rework_notice": "A problem was found during verification and is being fixed; steps marked ↻ will run again afterwards.",
