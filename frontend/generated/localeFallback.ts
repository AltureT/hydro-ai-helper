...
export const LOCALE_FALLBACK: Record<'zh' | 'en', Record<string, string>> = {
  "zh": {
    ...
    "ai_helper_testdata_generating_hint": "任务没有预设截止时间；深度思考模型可能需要较长时间，继续等待可能增加费用，您可以随时主动取消。",
    ...
    "ai_helper_testdata_progress_heartbeat": "后台任务运行正常 · 最近心跳 {0} 秒前",
    "ai_helper_testdata_progress_heartbeat_stale": "暂未收到最新后台心跳，页面正在继续查询任务状态。请勿重复启动新任务；若服务已中断，页面会显示明确结果。",
    "ai_helper_testdata_progress_independent_verifier": "AI 正在独立生成暴力解与压力数据",
    "ai_helper_testdata_progress_last_update": "距上次后台阶段更新 {0}:{1}",
    "ai_helper_testdata_progress_live": "服务端报告的实际阶段",
    ...
    "ai_helper_testdata_progress_model_unobservable": "模型在当前阶段内部思考了多少无法精确测量，因此这里只显示真实阶段和耗时，不显示猜测的完成百分比。",
    ...
    "ai_helper_testdata_progress_waiting_long": "这一阶段耗时较长，但后台心跳正常时任务仍在运行。系统不会因题目较难而主动终止；您可以继续等待或主动取消。",
  ...
  "en": {
    ...
    "ai_helper_testdata_generating_hint": "There is no preset job deadline. Deep-reasoning models may take a long time and continued waiting may increase costs; you can cancel the task at any time.",
    ...
    "ai_helper_testdata_progress_heartbeat": "Background task is healthy · last heartbeat {0}s ago",
    "ai_helper_testdata_progress_heartbeat_stale": "No recent background heartbeat has arrived yet. This page is still checking the task. Do not start a duplicate task; an explicit result will appear if the service was interrupted.",
    ...
    "ai_helper_testdata_progress_live": "Actual stage reported by the server",
    ...
    "ai_helper_testdata_progress_model_unobservable": "The model's internal completion within a stage cannot be measured accurately, so this view shows only real stages and elapsed time instead of a guessed percentage.",
    ...
    "ai_helper_testdata_progress_waiting_long": "This stage is taking a long time, but the task is still running while its background heartbeat is healthy. The system will not stop merely because the problem is difficult; you can keep waiting or cancel it.",
